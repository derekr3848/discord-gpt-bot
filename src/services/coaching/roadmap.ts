...
export async function setStage(userId: string, stageId: string): Promise<UserRoadmap | null> {
  ...
  const exists = roadmap.stages.find(
    (s: UserRoadmap["stages"][number]) => s.id === stageId
  );
  if (!exists) return null;

  roadmap.currentStageId = stageId;

  roadmap.stages = roadmap.stages.map(
    (s: UserRoadmap["stages"][number]) =>
      s.id === stageId
        ? { ...s, status: "active" }
        : s.status === "active"
        ? { ...s, status: "completed" }
...
export async function markTaskCompleted(
  ...
  stage.tasks = stage.tasks.filter((t: string) => t !== task);
